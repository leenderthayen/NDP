...
NDDDetectorMessenger::NDDDetectorMessenger(NDDDetectorConstruction* myDet)
  ...
  AlThickCmd = new G4UIcmdWithADoubleAndUnit("/NDD/geometry/AlumThickness", this);
  AlThickCmd->SetGuidance("Set the Aluminized Layer thickness.");
  AlThickCmd->AvailableForStates(G4State_PreInit, G4State_Idle);
...
void NDDDetectorMessenger::SetNewValue(G4UIcommand* command, G4String newValue) {
    ...
    } else if (command == AlThickCmd) {
        detector->SetAlumThickness(AlThickCmd->GetNewDoubleValue(newValue));
