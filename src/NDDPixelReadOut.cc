...
void NDDPixelReadOut::Construct() {
  ...
  G4VPhysicalVolume* physicalWorld = GetWorld();
  G4LogicalVolume* logicalWorld = physicalWorld->GetLogicalVolume();

  // G4VSolid* solidROSilicon = new G4Tubs("solidROSilicon", 0., siOuterRadius,
  //                               (siThickness + deadLayerThickness) / 2., 0., 360. * deg);
  // logicalROSilicon = new G4LogicalVolume(solidSilicon, dummyMat,
  //                                          "logicalROSilicon");
  // physicalROSilicon = new G4PVPlacement(
  //     0,
  //     G4ThreeVector(xSilicon, ySilicon, zSilicon),
  //     logicalSilicon, "physicalROSilicon", logicalWorld, false, 0);
  //
  ...
  //
  // solidPixel = new G4Polyhedron("solidROPixel", 0, 360. * deg, 6, 2,
  ...
  //                              pixelSize,
  //                          -siThickness / 2.0),
  //         logicalPixel, "SiROPixel", logicalSilicon, false, cn);
  //     cn++;
  //   }
  // }
}
